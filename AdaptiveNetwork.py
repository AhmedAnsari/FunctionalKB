...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 22 12:48:25 2017
...
with tf.device(DEVICE):
    ...
    pop_mat = tf.get_variable(name='POP',shape = [EMBEDDING_SIZE,\
                       EMBEDDING_SIZE],initializer = \
                        tf.constant_initializer(np.identity(EMBEDDING_SIZE)))
#    pop_bias = tf.get_variable(name='POPB',shape = [EMBEDDING_SIZE],initializer = \
#                        tf.constant_initializer(np.zeros(EMBEDDING_SIZE)))
    ...
    prev_pop_layer_out = tf.placeholder(tf.float32,[VOCABULARY_SIZE,EMBEDDING_SIZE])

    #this will contain embedding ids in given a batch
    X = tf.placeholder(tf.int32, shape=[BATCH_SIZE])
    
    Z = tf.placeholder(tf.int32, shape=[VOCABULARY_SIZE])
...
neg_h_e = tf.nn.embedding_lookup(embednew, neg_h)
neg_t_e = tf.nn.embedding_lookup(embednew, neg_t)
neg_r_e = tf.nn.embedding_lookup(rel_embeddings, neg_r)


eval_h_e = tf.nn.embedding_lookup(embednew, eval_h)
eval_t_e = tf.nn.embedding_lookup(embednew, eval_t)
eval_r_e = tf.nn.embedding_lookup(rel_embeddings, eval_r)
eval_to_rank_e = tf.matmul(tf.nn.embedding_lookup(ent_embeddings, eval_to_rank),pop_mat)
...
POP_LAYER = tf.nn.embedding_lookup(tf.matmul(ent_embeddings,pop_mat), Z)
...
optimizer1 = tf.train.AdamOptimizer(learning_rate=LEARNING_RATE,beta1=0.9,\
                                   beta2=0.999,epsilon=1e-08)

optimizer2 = tf.train.AdamOptimizer(learning_rate=LEARNING_RATE,beta1=0.9,\
                                   beta2=0.999,epsilon=1e-08)

optimizer3 = tf.train.AdamOptimizer(learning_rate=1e-3,beta1=0.9,\
                                   beta2=0.999,epsilon=1e-08)
