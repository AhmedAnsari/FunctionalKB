...
expected_t = Translate(pos_h_e,pos_r_e)
neg_expect_t = neg_Translate(neg_h_e,neg_r_e)
#evaluation part of TransE
_t, indices_t = predict_rank(expected_t, eval_to_rank_e, \
...
loss_regulariation = tf.reduce_sum(tf.stack(map(lambda x: 
 ...
 #TransE loss
pos = tf.reduce_sum((expected_t - pos_t_e) ** 2, 1, keep_dims = True)
neg_per_pos = tf.reduce_sum((neg_expect_t - neg_t_e) ** 2, 2, keep_dims = True)		
